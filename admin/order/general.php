...
include "../../client/DBUntil.php";
    ...
    $salesInDate =  $dbHelper->select("SELECT DATE(orderDate) AS order_day, SUM(totalPrice) AS total_revenue
                                       FROM orders
                                       WHERE statusOrder = '3' -- Giả sử chỉ tính các đơn hàng đã hoàn thành
                                       GROUP BY DATE(order_day)
                                       ORDER BY order_day");
    $salesCharts = $dbHelper->select("SELECT DATE(orderDate) AS day, COUNT(idOrder) AS total_orders, SUM(totalPrice) AS total_sales FROM orders GROUP BY day ORDER BY day");
        ...
        <div class="col-md-6">
            ...
            <div class="warning-products">
                ...
                <?php foreach ($products as $product) { ?>
                    <?php if ($product['quantityProduct'] < 3) { 
